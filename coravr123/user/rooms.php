 ...
 * ROOM IMAGE HANDLER
 * ------------------
 * Always use a REAL image file from assets/images/rooms/.
 * If the database contains an old/missing filename, automatically use
 * the verified local fallback instead of showing a broken-image icon.
 */
$rooms_image_dir = __DIR__ . '/../assets/images/rooms/';
$rooms_image_url = '../assets/images/rooms/';

/*
 * IMPORTANT: The four main accommodation images are tied to the room name,
 * not to whatever filename happens to be stored in the database. This stops
 * an old/wrong database filename from making two different rooms show the
 * same photo.
 */
$fixed_room_images = [
    'duplex room'      => 'duplex-room.jpg',
    'deluxe room'      => 'duplex-room.jpg', // supports existing DB name
    'family room'      => 'family-room.jpg',
    'small bahay kubo' => 'small-bahay-kubo.jpg',
    'large bahay kubo' => 'large-bahay-kubo.jpg',
];

/* Safe fallback: this file is confirmed to exist in assets/images/rooms/. */
$default_room_image_file = 'room_6a85b4f1dc5b9.jpg';
$default_img = $rooms_image_url . $default_room_image_file;
...
function resolveRoomImage($room_name, $filename, $image_dir, $image_url, $fixed_images, $fallback_file) {
    $normalized_name = normalizeRoomName($room_name);

    /* 1. Fixed accommodation mapping always wins. */
    if (isset($fixed_images[$normalized_name])) {
        $fixed_file = $fixed_images[$normalized_name];
        if (is_file($image_dir . $fixed_file)) {
            return $image_url . rawurlencode($fixed_file);
        }
    }

    /* 2. For any other room, use the database image only when the file exists. */
    $safe_filename = basename(trim((string)$filename));
    if ($safe_filename !== '' && is_file($image_dir . $safe_filename)) {
        return $image_url . rawurlencode($safe_filename);
    }

    /* 3. Final local fallback. */
    return $image_url . rawurlencode($fallback_file);
}
...
if ($rq) {
    while ($row = $rq->fetch_assoc()) {
        ...
        $rooms[] = [
            ...
            'img'         => resolveRoomImage($row['room_name'] ?? '', $row['image'] ?? '', $rooms_image_dir, $rooms_image_url, $fixed_room_images, $default_room_image_file),
...
// If we just redirected here after a successful insert, show the
// confirmation message. This is a plain GET request, so refreshing
// it is safe — nothing gets resubmitted.
    ...
    <div class="modal-box">
        ...
        <div class="cv-booking-left">
            <div class="cv-room-photo">
                <img class="modal-room-img-preview" id="modalImg" src="<?= htmlspecialchars($default_img) ?>" alt="Room" onerror="this.onerror=null;this.src='<?= htmlspecialchars($default_img, ENT_QUOTES) ?>';">
            </div>
    ...
    <div class="resort-hero-info">
        ...
        <div class="resort-hero-details">
            <a class="resort-hero-detail" href="https://www.google.com/maps/@10.714106,122.396162,16z" target="_blank" rel="noopener noreferrer">
                <i class="fa-solid fa-building"></i>Barosong, Tigbauan, Iloilo City, Philippines
            </a>
            <a class="resort-hero-detail" href="https://www.facebook.com/coravergelresort"  rel="noopener noreferrer">
            ...
            <div class="search-bar-container_top">
                ...
                <div class="search-bar-container_guestsWrapper">
                    ...
                    <div class="guests-flyout-container" id="guestsFlyoutContainer" onclick="event.stopPropagation()">
                        ...
                        <div class="g-row" style="justify-content:flex-end;border-top:1px solid #e5e5e5;margin-top:4px;padding-top:10px;">
                            <button type="button" class="cal-done" onclick="gApplyGuests()">Apply</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="room-card <?= ($over_cap || $fully_booked) ? 'room-card--dimmed' : '' ?>">
                <div class="room-img" onclick="openRoomLightbox('<?= addslashes($r['id']) ?>', '<?= addslashes($r['img']) ?>')">
                    <img src="<?= htmlspecialchars($r['img']) ?>" alt="<?= htmlspecialchars($r['id']) ?>"
                         onerror="this.onerror=null;this.src='<?= htmlspecialchars($default_img, ENT_QUOTES) ?>';">
                ...
                <div class="room-main">
                    <div class="room-info">
                        ...
                        <div class="room-meta">
                            <?php if ($recent_hours !== null && $recent_hours < 48): ?>
                            <span class="room-meta-status">Booked in last <?= $recent_hours < 1 ? 'hour' : $recent_hours . ' hour' . ($recent_hours != 1 ? 's' : '') ?></span>
                            <?php endif; ?>
                            <?php if ($r['cap']): ?>
                            <?php if ($recent_hours !== null && $recent_hours < 48): ?><span class="room-meta-sep">&middot;</span><?php endif; ?>
                            <span><?= $r['cap'] ?> Pax</span>
                            <?php endif; ?>
                            <?php if ($has_dates && !$fully_booked && !$over_cap && $available_units !== null && $available_units <= 2): ?>
                            <span class="room-meta-sep">&middot;</span>
                            <span class="room-meta-status"><i class="fa-solid fa-triangle-exclamation"></i> Only <?= $available_units ?> unit<?= $available_units!=1?'s':'' ?> left</span>
                            <?php endif; ?>
                        </div>
...
<footer class="site-footer">
    ...
    <div class="footer-links">
        ...
        <div class="footer-col footer-contact-col">
            ...
            <a href="tel:639629544504" class="topbar-link">+63 962 954 4504</a>
...
const DEFAULT_ROOM_IMAGE = <?= json_encode($default_img) ?>;

function setSafeImage(imgEl, src) {
    if (!imgEl) return;
    imgEl.onerror = function() {
        this.onerror = null;
        this.src = DEFAULT_ROOM_IMAGE;
    };
    imgEl.src = src || DEFAULT_ROOM_IMAGE;
}
...
// Pushes the current room/adult/child counts into the URL (same
// pattern as calDone() for the calendar). Without this, +/- only
// updates the on-screen label and local JS variables — nothing the
// server sees — so a reload falls back to whatever "rooms"/"adults"/
// "children" were in the URL at page load.
function gApplyGuests(){
    const p = new URLSearchParams({
        check_in: URL_CI,
        check_out: URL_CO,
        guests: adultCount + childCount,
...
function openModal(roomId, price, img) {
    ...
    setSafeImage(document.getElementById('modalImg'), img);
...
function openRoomLightbox(roomName, img) {
    setSafeImage(document.getElementById('roomLightboxImg'), img);
