...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    if (empty($username)) {
        ...
        if ($lockout_seconds > 0) {
            ...
            $has_otp_column = true;
            try {
                $stmt = $conn->prepare("SELECT admin_id, full_name, email, password, otp_enabled, otp_email FROM admins WHERE username = ?");
                $stmt->bind_param("s", $username);
                $stmt->execute();
                $stmt->store_result();
            } catch (mysqli_sql_exception $e) {
                // otp_enabled column not migrated yet — fall back so login
                // still works; OTP will simply run every time until it's added.
                $has_otp_column = false;
                $stmt = $conn->prepare("SELECT admin_id, full_name, email, password, otp_email FROM admins WHERE username = ?");
                $stmt->bind_param("s", $username);
                $stmt->execute();
                $stmt->store_result();
            }
            if ($stmt->num_rows === 1) {
                if ($has_otp_column) {
                    $stmt->bind_result($id, $full_name, $email, $hashed_password, $otp_enabled, $otp_email);
                } else {
                    $stmt->bind_result($id, $full_name, $email, $hashed_password, $otp_email);
                    $otp_enabled = 1;
                }
                $stmt->fetch();
                if (password_verify($password, $hashed_password)) {
                    ...
                    // Admin has turned off email OTP for their own account in
                    // Settings — sign them straight in on password alone.
                    if ((int)$otp_enabled === 0) {
                        session_regenerate_id(true);
                        $_SESSION['admin_id']   = $id;
                        $_SESSION['admin_name'] = htmlspecialchars($full_name, ENT_QUOTES, 'UTF-8');
                        $_SESSION['admin_role'] = 'admin';
                    ...
                    $otp = str_pad(random_int(0, 999999), 6, '0', STR_PAD_LEFT);
                    $otp_expires = date('Y-m-d H:i:s', strtotime('+10 minutes'));
                    $delivery_email = !empty($otp_email) ? $otp_email : $email;
                    ...
                    // Respect the OTP delivery address configured in Settings.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'verify_otp') {
    ...
    if (!isset($_SESSION['temp_admin_email'])) {
       ...
       $stmt = $conn->prepare("SELECT otp, expires_at FROM otp_codes WHERE email = ? ORDER BY otp_id DESC LIMIT 1");
        $stmt->bind_param("s", $email); $stmt->execute();
